...
-- Create a local test database and user

-- Create database if it doesn't exist
CREATE DATABASE IF NOT EXISTS mejona_unified;

-- Use the database
USE mejona_unified;

-- Create a test user (for local development only)
-- Note: In production, use the existing mejona_contact user
CREATE USER IF NOT EXISTS 'test_user'@'localhost' IDENTIFIED BY 'test_password';
GRANT ALL PRIVILEGES ON mejona_unified.* TO 'test_user'@'localhost';
FLUSH PRIVILEGES;

-- Create contacts table with complete CRM structure
...
SHOW TABLES;
SELECT COUNT(*) as contact_types_count FROM contact_types;
SELECT COUNT(*) as contact_sources_count FROM contact_sources;